...
experiment(S,N,J,K,[Ps,Pos,Neg,Acc,TPR,TNR]):-
        generate_initial(even,N,J,K,Es)
        ,debug_clauses(experiment_full,'Generated new examples:',Es)
        ,debug_length(experiment_full,'Generated ~w new examples:',Es)
        ,time( learn(Es,Pos,Neg,Ps) )
        ,debug_clauses(experiment_full,'Learned hypothesis:',Ps)
        ,accuracy(S,Pos,Neg,Acc)
        ,tpr(S,Pos,TPR)
        ,tnr(S,Neg,TNR)
        ,debug(experiment,'Measured Acc: ~w TPR: ~w TNR: ~w',[Acc,TPR,TNR]).
