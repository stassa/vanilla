...
%
program_reduction(Ps, Rs, Ds):-
	louise_configuration:program_module(M)
	,assert_program(M,Ps)
...
%
subsumed(M,C):-
	louise_configuration:derivation_depth(D)
	,louise_configuration:recursion_depth(R)
	,louise_configuration:resolutions(S)
	,louise_configuration:call_limit(L)
	,(   C = (H:-B)
	 ->  true
	 ;   (H:-B) = (C:-true)
	 )
	...
	,(   louise_configuration:meta_interpreter(solve_to_depth)
	 ->  G = solve_to_depth(M,H_,D, S)
	 ;   louise_configuration:meta_interpreter(solve_to_limit)
	 ->  G = solve_to_limit(M,H_,D,R)
	 ;   louise_configuration:meta_interpreter(solve)
	 ->  G = solve(M, H_)
	 ;   louise_configuration:meta_interpreter(call)
	 ->  G = call(M:H_)
	 ;   louise_configuration:meta_interpreter(MI)
	    ,retract_goals(M, B_)
...
limited_subsumed(time, M, G, B):-
	!
	,louise_configuration:time_limit(L)
...
limited_subsumed(L, M, G, B):-
	louise_configuration:depth_limit(D)
	,louise_configuration:inference_limit(I)
	,(   L == depth
