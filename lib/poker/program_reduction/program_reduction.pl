...
%
program_reduction(Ps, Rs, Ds):-
	poker_configuration:program_module(M)
	,assert_program(M,Ps)
...
%
subsumed(M,C):-
	poker_configuration:derivation_depth(D)
	,poker_configuration:recursion_depth(R)
	,poker_configuration:resolutions(S)
	,poker_configuration:call_limit(L)
	,(   C = (H:-B)
	 ->  true
	 ;   (H:-B) = (C:-true)
	 )
	...
	,(   poker_configuration:meta_interpreter(solve_to_depth)
	 ->  G = solve_to_depth(M,H_,D, S)
	 ;   poker_configuration:meta_interpreter(solve_to_limit)
	 ->  G = solve_to_limit(M,H_,D,R)
	 ;   poker_configuration:meta_interpreter(solve)
	 ->  G = solve(M, H_)
	 ;   poker_configuration:meta_interpreter(call)
	 ->  G = call(M:H_)
	 ;   poker_configuration:meta_interpreter(MI)
	    ,retract_goals(M, B_)
...
limited_subsumed(time, M, G, B):-
	!
	,poker_configuration:time_limit(L)
...
limited_subsumed(L, M, G, B):-
	poker_configuration:depth_limit(D)
	,poker_configuration:inference_limit(I)
	,(   L == depth
